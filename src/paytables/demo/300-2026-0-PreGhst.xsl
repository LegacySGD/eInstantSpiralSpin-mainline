			...
			<lxslt:component prefix="my-ext" functions="formatJson">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNames)
					{
						...
						{
							symbolPotData.push(new Pot(prizes[i]));
						}						
						...
						r.push('<td>');
						r.push(getTranslationByName("wedge", translations));
						r.push('</td>');
						r.push('<td>');
						...
						for(var i = 0; i < symbolPotData.length; i++)
						{
							r.push('<tr>');
							var matchedString = "";
							if(symbolPotData[i].count == 3)
							{
								matchedString = getTranslationByName("youMatched", translations) + ": ";
							}							
							r.push('<td class="tablebody">');
							r.push(matchedString + convertedPrizeValues[prizes.indexOf(symbolPotData[i].letter)]);
							r.push('</td>');
							r.push('<td class="tablebody">');
